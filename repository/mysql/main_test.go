...
func startMySQL() (*gorm.DB, string, func(), error) {
	...
	runOptions := &dockertest.RunOptions{
		...
		Env: []string{
			...
			"MYSQL_ROOT_PASSWORD=go-chat-app",
			"MYSQL_DATABASE=go_chat_app_test_db",
		},
		Cmd: []string{
	...
	err = pool.Retry(func() error {
		dsn := fmt.Sprintf("root:go-chat-app@(localhost:%s)/go_chat_app_test_db?charset=utf8mb4&parseTime=True", port)
